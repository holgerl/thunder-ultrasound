...
void fill_voxels(int intersection_counter) {
	...
	for (int i = 0; i < intersection_counter; i++) {
		...
		int safety = 0;
		for (int z = min(intrs0.z,intrs1.z); z <= max(intrs0.z,intrs1.z); z++)
			for (int y = min(intrs0.y,intrs1.y); y <= max(intrs0.y,intrs1.y); y++)
				for (int x = min(intrs0.x,intrs1.x); x <= max(intrs0.x,intrs1.x); x++) {
...
void reconstruct() {
	...
	while(true) {
		...
		int axis = 2;																							// Actually, turns out that the output is pretty much equal for any axis, 
