...
extern int volume_w, volume_h, volume_n;
...
int bitmap_size = sizeof(unsigned char)*bitmap_w*bitmap_h;
...
void display(void) {
	...
	if (gui_mode == RAY_CASTING) {
		...
		float x_pos = camera_pos[0]-camera_lookat[0];
		float y_pos = camera_pos[2]-camera_lookat[2];
		camera_pos[0] = cos(rot_angle)*x_pos - sin(rot_angle)*y_pos + camera_lookat[0];
		camera_pos[2] = sin(rot_angle)*x_pos + cos(rot_angle)*y_pos + camera_lookat[2];

		//rot_angle = -mouse_rot_y/180.0f*3.14/5.0f;
		//x_pos = camera_pos[1]-camera_lookat[1];
		//y_pos = camera_pos[2]-camera_lookat[2];
		//camera_pos[1] = cos(rot_angle)*x_pos - sin(rot_angle)*y_pos + camera_lookat[1];
		//camera_pos[2] = sin(rot_angle)*x_pos + cos(rot_angle)*y_pos + camera_lookat[2];
		
		static int i = 0;
		if (i++ % 4 == 0) // simple trick to make ray casting not use too much of the gpu cycles
	...
	if (gui_mode == MPR_SLICES) {
		...
		reset_gl_matrices();
		gluPerspective(30.0f, win_width/(float)win_height, 0.1f, 10000.0f);
...
void key(unsigned char key, int x, int y) {
  if (key == '\033' || key == 'q')
	...
	if (key == 'r')
		if (bscan_index <= bscan_n-10) bscan_index += 10;
	if (key == 'f')
		if (bscan_index >= 10) bscan_index -= 10;
...
void profiler_gui() {
	holger_time_start(2, "Ray casting");
